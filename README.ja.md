...
> 完全ローカル実行・外部依存ゼロ・決定論的（100%再現可能）なテキスト構造・盗用スクリーニングエンジンです。[Sieve-Core](https://github.com/neguseatama/sieve-core) の拡張応用プラグインとして開発しました。

---

## 💡 コンセプトと開発思想
「エラトステネスの篩」に着想を得て設計した「集合論の篩」 Sieve-AI の設計思想を、より汎用的なテキスト・データ処理ドメインに適用したコアエンジンが Sieve-Core です。
その Sieve-Core の拡張応用事例として新たに「組み合わせ論の篩」を追加設計した　`Sieve Referee` は、複数の独立した情報源による裏付けに基づき、ノイズから確かな情報を抽出するゼロディペンデンシーのPythonライブラリです。
